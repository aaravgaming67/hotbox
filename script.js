...
function setDefaultSlot() {
  ...
  const hoursValue = String(slot.getHours()).padStart(2, "0");
  const minutesValue = String(slot.getMinutes()).padStart(2, "0");
...
function bookingCode() {
  return `HB-${Math.random().toString(36).slice(2, 6).toUpperCase()}-${Date.now().toString().slice(-4)}`;
}
